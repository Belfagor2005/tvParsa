#!/bin/bash
## setup command=wget -q --no-check-certificate https://raw.githubusercontent.com/Belfagor2005/tvParsa/main/installer.sh -O - | /bin/sh

version='1.8'
changelog='\nRecoded PEP8'

TMPPATH=/tmp/tvParsa-install
FILEPATH=/tmp/tvParsa-main.tar.gz

echo "Starting tvParsa installation..."

# Determine plugin path based on architecture
if [ ! -d /usr/lib64 ]; then
    PLUGINPATH=/usr/lib/enigma2/python/Plugins/Extensions/parsatv
else
    PLUGINPATH=/usr/lib64/enigma2/python/Plugins/Extensions/parsatv
fi
...
cleanup() {
    ...
    [ -d "/tmp/tvParsa-main" ] && rm -rf "/tmp/tvParsa-main"
}
...
echo "⬇️ Downloading tvParsa..."
wget --no-check-certificate 'https://github.com/Belfagor2005/tvParsa/archive/refs/heads/main.tar.gz' -O "$FILEPATH"
if [ $? -ne 0 ]; then
    echo "❌ Failed to download tvParsa package!"
    cleanup
    exit 1
fi
...
if [ $? -ne 0 ]; then
    echo "❌ Failed to extract tvParsa package!"
    cleanup
    exit 1
fi
...
if [ -d "$TMPPATH/tvParsa-main/usr/lib/enigma2/python/Plugins/Extensions/parsatv" ]; then
    cp -r "$TMPPATH/tvParsa-main/usr/lib/enigma2/python/Plugins/Extensions/parsatv"/* "$PLUGINPATH/" 2>/dev/null
    echo "✅ Copied from standard plugin directory"
elif [ -d "$TMPPATH/tvParsa-main/usr/lib64/enigma2/python/Plugins/Extensions/parsatv" ]; then
    cp -r "$TMPPATH/tvParsa-main/usr/lib64/enigma2/python/Plugins/Extensions/parsatv"/* "$PLUGINPATH/" 2>/dev/null
    echo "✅ Copied from lib64 plugin directory"
elif [ -d "$TMPPATH/tvParsa-main/usr" ]; then
    # Copy entire usr tree
    cp -r "$TMPPATH/tvParsa-main/usr"/* /usr/ 2>/dev/null
